...
//            this.keyboardListener = new window.keypress.Listener();
        ...
        launch: function () {
            ...
            ********************/
            ...
            var c_outer_points = c_outer.getEquallySpacedPoints(num_petals);
            var c_outer_points_lines = geo.connectEveryN(c_outer_points, 3);
            for(i=0; i<c_outer_points_lines.length; i++){
                var pts = geo.getAllIntersections(c_outer_points_lines[i]);
            }

            var c_middle = geo.circleFromPoint(p1,100);
            var c_middle_points = c_middle.getEquallySpacedPoints(num_petals);

            var c_inner = geo.circleFromPoint(p1,50);
            var c_inner_points = c_inner.getEquallySpacedPoints(num_petals);
            console.log(c_middle_points);
            console.log(c_inner_points);
            var ray_lines = []
            for(i=0; i<num_petals; i++){
                ray_lines.push(geo.lineFromPoints(c_middle_points[i], c_inner_points[(i+1)%num_petals]));
                ray_lines.push(geo.lineFromPoints(c_middle_points[i], c_inner_points[(i-1 + num_petals)%num_petals]));
